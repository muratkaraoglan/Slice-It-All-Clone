...
public class MeshSlicer : MonoBehaviour
{
    ...
    public void Slice(Plane plane, Vector3 pointOnPlane)
    {
        ...
        if ( sliceable )
        {
            ...
            negative.tag = "Untagged";
            positive.tag = "Untagged";
            // reset negative mesh
            ...
            // draw interior faces
            Material mat = new Material(innerMaterial);
            mat.color = randomColor ? colors[UnityEngine.Random.Range(0, colors.Length)] : innerMaterial.color;
            negativeSlicer.DrawInteriorFaces(plane.normal, mat);
            positiveSlicer.DrawInteriorFaces(-plane.normal, mat);
            ...
            Rigidbody rbPos = positive.AddComponent<Rigidbody>();
            rbPos.AddForce(Vector3.right * speed);
            rbPos.AddTorque(Vector3.back * rotSpeed);
            positiveMeshCollider.convex = true;

            Rigidbody rbNeg = negative.AddComponent<Rigidbody>();
            rbNeg.AddForce(Vector3.left * speed);
            rbNeg.AddTorque(Vector3.forward * rotSpeed);
            negativeMeshCollider.convex = true;

            negativeSlicer.sliceable = false;
            positiveSlicer.sliceable = false;
    ...
    protected void DrawInteriorFaces(Vector3 normal, Material innerMat)
    {
